...
# transcript state directly via the MCP server as a safety net.
...
Context compaction is about to happen. You are about to lose most of your conversation history. You MUST store a comprehensive session summary NOW using the mem0 `add_memories` MCP tool.

### Step 1: Store session summary

Call `add_memories` with a thorough summary covering ALL of the following:
