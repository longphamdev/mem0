...
# process to capture transcript state via the MCP server directly.
#
...
cat <<'EOF'
Before finishing, check if there are important learnings from this interaction that should be persisted using the mem0 MCP `add_memories` tool:
...
# Capture transcript state in the background via MCP server
